...
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        List<Booking> bookings = List.of(
                new Booking(null, LocalDate.of(2025, 8, 10), LocalDate.of(2025, 8, 14), 130.0, 2, rooms.subList(0, 1),
                        user),
                new Booking(null, LocalDate.of(2025, 8, 25), LocalDate.of(2025, 8, 26), 140.0, 5, rooms.subList(0, 2),
                        user));

        bookings.stream().forEach(item -> em.persist(item));

        roomsId = rooms.stream().map(r -> r.getId()).toList();
    }
