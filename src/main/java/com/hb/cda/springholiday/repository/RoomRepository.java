...
public interface RoomRepository extends JpaRepository<Room, String>{
    ...
    @Query("SELECT count(b) = 0 FROM Room r JOIN r.bookings b WHERE b.startDate < :end AND b.endDate >:start")
    boolean isRoomAvailable(Room room, LocalDate start, LocalDate end);

    @Query("FROM Room r WHERE r NOT IN (SELECT b.rooms FROM Booking b WHERE b.startDate < :end AND b.endDate >:start)")
